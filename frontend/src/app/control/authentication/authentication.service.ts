...
import {
  AuthUser,
  AuthResponse
} from '../../model/entity/authentication.model';
...
export class AuthenticationService {
  ...
  public doLoginFacebook() {
    this.authService
      ...
      .then(
        socialUser => this.login(socialUser),
        error => this.errorOnSocialProfileCredentials(FACEBOOK_STRING, error)
      )
      .catch(error =>
        this.errorOnSocialProfileConnection(FACEBOOK_STRING, error)
      );
  }
  ...
  public doLoginGoogle() {
    this.authService
      ...
      .then(
        socialUser => this.login(socialUser),
        error => this.errorOnSocialProfileCredentials(GOOGLE_STRING, error)
      )
      .catch(error =>
        this.errorOnSocialProfileConnection(GOOGLE_STRING, error)
      );
  }
  ...
  public getValidUserAtLocalStorage(): AuthUser {
    return JSON.parse(
      localStorage.getItem(SettingsService.LOGGED_USER)
    ) as AuthUser;
  }
  ...
  private errorOnSocialProfileCredentials(socialPlataform: string, error: any) {
    ...
    this.notificationService.error(
      'Ops!',
      'Sua conta de ' + socialPlataform + ' não é válida. Tente novamente.'
    );
  }
  ...
  private errorOnSocialProfileConnection(socialPlataform: string, error: any) {
    ...
    this.notificationService.error(
      'Ops!',
      'Houve um erro de comunicação com o ' +
        socialPlataform +
        '. Tente mais tarde.'
    );
  }
